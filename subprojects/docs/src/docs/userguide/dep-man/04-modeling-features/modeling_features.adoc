...
We have explained in this chapter that in reality `A` depends on a _variant of_ `B` and that this variant depends on the needs of `A`.
...
However, what if you need a _different_ artifact that the main one?
...
====
It is frequent to see cross-project dependencies declared like this:
...
This section explains how to _properly create cross-project boundaries_ by defining "exchanges" between projects.
Those "exchanges" are precisely _variants_.
====
...
It also is only limited to cases where this artifact shouldn't be published on a repository, and that the consumer itself wouldn't have a published dependency on this artifact.
In case publication is required or that you need to resolve a _different artifact_ depending on the consumer (for example, different target platforms), you need to use the <<#sec:variant-aware-sharing,advanced version>>.

[[sec:simple-sharing-artifacts-between-projects]]
=== Simple sharing of artifacts between projects

First, a producer needs to declare a configuration which is going to be _exposed to consumers_.
...
Let's imagine that the _consumer_ requires _instrumented classes_ from the producer, but that this artifact is _not_ the main one.
The _producer_ can expose its instrumented classes by creating a configuration that will "carry" this artifact:
