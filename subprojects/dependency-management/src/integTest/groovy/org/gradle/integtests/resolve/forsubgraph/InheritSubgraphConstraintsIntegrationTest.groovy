...
class InheritSubgraphConstraintsIntegrationTest extends AbstractModuleDependencyResolveTest {
    ...
    void "can use dependency constraint to downgrade version"() {
        ...
        resolve.expectGraph {
            root(':', ':test:') {
                ...
                module('org:bar:1.0') {
                    edge('org:foo:2.0', 'org:foo:1.0').byConstraint().byParent()
                }
            }
        }
    }
    ...
    void "a forSubgraph constraint wins over a nested forSubgraph constraint"() {
        ...
        resolve.expectGraph {
            root(':', ':test:') {
                module('org:a:1.0') {
                    module('org:b:1.0') {
                        edge('org:c:3.0', 'org:c:1.0').byParent()
                    }
    ...
    void "identical forSubgraph constraints can co-exist in a graph"() {
        ...
        resolve.expectGraph {
            root(':', ':test:') {
                module('org:a:1.0') {
                    module('org:b:1.0') {
                        edge('org:c:2.0', 'org:c:1.0').byParent()
                    }
                    if (publishedConstraintsSupported) {
            ...
            root(':', ':test:') {
                module('org:a:1.0') {
                    ...
                    edge('org:c:1.0', cResult).byRequest().with {
                        if (cResult == 'org:c:1.0') { it.byParent() } else { it.byConflictResolution("between versions 2.0 and 1.0") }
                    }
                }
            }
        }
